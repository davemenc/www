...
function loc_display_login($title,$color,$mode,$message="",$loginapp="index.php",$reg=false,$PARAMS=""){
	global $version,$lastmodified;	
	debug_string("loc_display_login($title,$color,$mode,$message,$loginapp,$reg)");
	$username = "";
	if ($PARAMS!=""){
	...
	echo <<< EOF
	<center>|  <a href="$loginapp?mode=list">Return to $title </a> |</center>
	<br><br>
	<br><br>
<center>
<table >
...
$usertable: the name of the table with the users in it

*******************************************/
function loc_GetAuthenticated($PARAMS,$link,$magic_word,$cookiename="login",$admin=false,$expiry=0,$title="Application",$color='#F0F0F0',$loginapp="index.php", $reg=false,$usertable = "user"){ 
	//debug_string("loc_GetAuthenticated(PARAMS,link,$magic_word,$cookiename,$admin,$expiry,$title,$color,$loginapp, $reg,$usertable)");
    //check Cookie
    //debug_string("---- loc_GetAuthenticated ----");
//debug_array("loc_GetAut PARAM",$PARAMS);
	if (isset($PARAMS['username'])) $username = $PARAMS['username'];
	else $username ="";
	if (isset($PARAMS['password'])) $pass = $PARAMS['password'];
	else $pass = "";
	...
	if (count($c_data)>0 && loc_check_auth_cookie($magic_word,loc_calc_cookie_expiry($expiry,$c_data['time']),$cookiename)){
...
// no valid cookie, no valid type, 
		...
		debug_string("no cookie or bad cookie");
		loc_delete_cookie($cookiename);
	}//if loc_check_auth
    debug_string("at this point there isn't a valid cookie");


    // check username/password
    if(isset($username) && isset($pass) && $username!="" && $pass!=""){//we have no cookie but we have a username and password
		debug_string("we have no cookie but we have a username and password");
    	$users = MYSQLComplexSelect($link,array("*"),array($usertable),array("username='".$username."'"),array(),0);
