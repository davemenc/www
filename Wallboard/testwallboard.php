...
$holdtime = 25;// how many seconds to keep the same set of URLs on the screen
...
$times=array("AM","PM");
$types=array("F","S");
for ($venue=1;$venue<=4;$venue++){
		foreach($times as $time){
			foreach($types as $type){
				$testmatrix[]=array('venue'=>$venue,'time'=>$time,'type'=>$type);
			}
		}
}
...
$time = $test['time'];
$urls = StripOutWrongTime($urls,$time);
...
$count = count($urls);
$info= "Shape: $shape; count: $count; Venue: $venue; Type: $type</br>\n";
...
switch ($shape){
        case "1": // this one is narrow and short, do 4 total
                ...
                for($i=0; $i<$count;$i++){
                  ...
                  $info.= $mainurls[0]['URL'].", ".$mainurls[1]['URL'].", ".$mainurls[2]['URL'].", ".$mainurls[3]['URL']."</br>\n";
