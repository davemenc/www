...
$rulelen = 19;
$startwidth = 7;
$startlen = $startwidth*$startwidth;// its square
$startcountwidth =8;
$startcountheight = $startcountwidth;
$genelen = $rulelen+$startlen;
...
//		print "rating<br>";
		...
		RateGene($link,$currentrun,$currentgeneration,$rulelen,$startwidth);
		break;
	case "rategene":
...
/***********************************************
 ***********************************************/
function RateGene($link,$run,$generation,$rulelen,$startwidth){
	global $startcountheight,$startcountwidth;
	...
	$startstring = GetStartFromGene($gene,$rulelen,$startwidth);
...
<body>
	...
	<b>Run # $run | Generation # $generation | Gene # $id | Done: $percentdone%</b>  <br>
 	<a href="info.html"><b>INFORMATION</b></a>  
...
function GetRulesFromGene($gene,$rulelen){
	...
	$len = strlen($rule);
	$rule = substr($rule,0,$len-1);//remove the final comma
	$code = "var rules = [$rule];";
	return($code);
 ...
 /***********************************************
  ***********************************************/
 function GetStartFromGene($gene,$rulelen,$startwidth){
	$datastr = substr($gene,$rulelen+1);// the +1 is to skip the leading non numeric character
	$xlist = "";
	$ylist = "";
	for($x=0;$x<$startwidth;$x++){
		for($y=0;$y<$startwidth;$y++){
			$val = substr($datastr,$y*($startwidth)+$x,1);
			if ($val=="1"){
				$xlist.="$x,";
				$ylist.="$y,";
			}
